...
This phase covers the configuration and build setups required before releasing **StudyLink** to production. It implements Docker build configurations, environment variable strategies, Cloud Run setup scripts, Vercel monorepo configurations, and the CORS handshake protocol.
...
### 1.2 Purpose
Configures the code workspace for serverless container deployment on Google Cloud Run and static hosting on Vercel.
...
- **Task Description:** Create `backend/config/settings_prod.py` (or add conditional checks in `settings.py`). Configure settings:
  - `DEBUG = False`
  - `ALLOWED_HOSTS = ['*']` (or domain specific)
  - `SECURE_SSL_REDIRECT = True`
  - `SESSION_COOKIE_SECURE = True`
  - `CSRF_COOKIE_SECURE = True`
  - Read secrets (database URL, API keys, OAuth credentials) from environment variables.
- **Definition of Done:**
...
- **Task Description:** Install `django-cors-headers`. In `settings.py`, configure `CORS_ALLOWED_ORIGINS` to read Vercel production and preview domains from environment variables.
- **Definition of Done:**
...
- **Task Description:** Configure `VITE_API_BASE_URL` in the frontend build settings to point to the Cloud Run backend URL.
- **Definition of Done:**
...
sequenceDiagram
    ...
    Note over CR: Spin up container & assign URL<br/>(e.g., https://studylink-backend-run.app)
    CR-->>Dev: Return Backend URL

    Dev->>V: Configure Vite API Variable
    Note over V: Set VITE_API_BASE_URL = Backend URL
    Dev->>V: Deploy Frontend Assets
    Note over V: Deploy assets & assign URL<br/>(e.g., https://studylink.vercel.app)
    V-->>Dev: Return Frontend URL

    Dev->>CR: Update CORS Origins
    Note over CR: Set CORS_ALLOWED_ORIGINS = Frontend URL
